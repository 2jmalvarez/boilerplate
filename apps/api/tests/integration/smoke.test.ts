...
describeWithDatabase('API smoke', () => {
  ...
  it('registers a user and manages an isolated task', async () => {
    const registration = await request(app)
      ...
      .send({ name: 'Integration User', email, password: 'correct-horse-battery-staple' })
      .expect(201);

    const registrationBody = registration.body as { data: { accessToken: string } };
    ...
    const creation = await request(app)
      ...
      .send({ title: 'Verify boilerplate', status: 'todo', dueDate: '2030-01-15' })
      .expect(201);

    const creationBody = creation.body as { data: { id: string; title: string } };
